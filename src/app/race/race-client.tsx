...
import { RaceRankingEntry } from "@/lib/mock-data-race";
...
export function RaceClient({ summary, initialRankings, lastUpdated }: RaceClientProps) {
  ...
  const activeFiltersCount = (nationalityFilter !== "All" ? 1 : 0) + (ageFilter !== "All" ? 1 : 0);
    ...
    <>
      ...
      <div className="mb-6">
        ...
        <p className="text-body-lg text-deep-navy/70 font-medium flex items-center gap-2">
          <span className="w-1 h-5 bg-[#DFFF00] rounded-full shrink-0" />
          <span>The top 8 singles players of the calendar year qualify for the prestigious season finale in Turin, Italy.</span>
        </p>
      </div>
      ...
      <div className="w-full bg-white rounded-3xl shadow-ambient border border-border-subtle overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between p-4 md:px-6 border-b border-border-subtle bg-white gap-4">
          <h2 className="text-headline-md text-deep-navy font-bold">Race Standings</h2>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {/* Updated Badge */}
            {lastUpdated && (
              <div className="rounded-full border border-border-subtle bg-white px-5 py-2.5 text-sm text-text-muted hidden md:block">
                Updated:{" "}
        ...
        {/* Race Rankings Table */}
        <RaceTable entries={filteredRankings} initialCount={20} />
      </div>
    </>
  );
}
